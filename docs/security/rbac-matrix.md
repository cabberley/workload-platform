...
three. Configuration values (a webhook URL, a read token, a subscription id) are, however, **read
directly from environment variables today** (`cfg` defaults to `os.environ` —
[`wiring.py`](../../src/cli/wiring.py)). **Key Vault-backed secret injection is not implemented:** the
Bicep sets only `AZURE_CLIENT_ID`/`WP_MODULE` plus plain `env` values and declares **no** Key Vault
`secretRef`/`secrets` block ([`module-app.bicep`](../../infra/bicep/modules/module-app.bicep)). The
**Key Vault Secrets User** role assignment (on `api`/`worker`) exists in Bicep but is
**provisioned-but-currently-unused** — see that row (tracked **#85**).
...
| **api** (service) | `identityApi` | *(intended)* The **single writer**: dispatch work to queues, read runtime secrets, **write state** (Blob/Table) — *the roles below are **deployed**, but these data-flows are **not yet wired** today: state persistence defaults to the **LOCAL** SQLite backend (§api status), **no client enqueues** to the queues (only KEDA polls depth), and **KV secret injection is unused** (#85)* | **AcrPull** (`7f951dda-…`) · **Storage Queue Data Contributor** (`974c5e8b-…`) · **Key Vault Secrets User** (`4633458b-…`) · **Storage Blob Data Contributor** (`ba92f5b4-…`) · **Storage Table Data Contributor** (`0a9a7e1f-…`) | AcrPull = the ACR; Queue/Blob/Table = the platform storage account; KV Secrets User = the platform Key Vault | The API is the (intended) sole state-writer, so it is granted the Blob + Table Data Contributor data roles (Contributor ⊇ Reader; the store creates and updates blobs/entities). Queue Data Contributor covers KEDA depth reads **and** the *intended* enqueue (no producer client enqueues today). KV Secrets User (get/list only) for the *intended* config injection (unused today — #85). AcrPull to pull its image. Nothing narrower writes state. | Least privilege · keyless · provenance | ✅ *(role assignments deployed #80; consuming flows ⚠️: Azure state backend defaults local — R6/#81; no enqueue client; KV unused — #85)* |
...
| **Packs Engine — signing key** ([`signing.py`](../../src/shared/signing.py)) | *(runtime identity — pending)* | Sign/verify pack canonical bytes | **Key Vault Crypto User** *(pending, contingent on #89)* | the platform Key Vault signing key only | **The current KV signer/verifier are stubs, and the recorded "KV Ed25519 keyless" design is not viable: Azure Key Vault Keys does not support Ed25519.** Reconciling the trust root is tracked by **#89** (options: **customer-side verification-only** with distributed Ed25519 **public** keys + offline Microsoft signing — needing *no* KV key op and *no* KV role at runtime; **or** switch to **ECDSA P-256** for KV-HSM signing — for which **Key Vault Crypto User** would be the narrowest role, **not** Secrets User). Pack import/assign is #37; digest-addressed content store + digest resolution is #44. | Detached pack signing · keyless · least privilege | 🔒 (#89) |

> **Signing-key note.** Issue #61 framed the pending signing key as *Key Vault Secrets User*. The
> code path ([`signing.py`](../../src/shared/signing.py)) *attempts* a Key Vault **key** operation
> (`CryptographyClient.sign(SignatureAlgorithm.eddsa, ...)`) — but this is a **stub**, and **Azure
> Key Vault Keys does not support Ed25519**, so the recorded design is not viable as written.
> Reconciling the trust root (Ed25519 public-key **verification-only** with offline signing, *or*
> ECDSA P-256 for KV-HSM signing) and wiring the verifier into the runtime is tracked by **#89**.
> *If* the ECDSA-in-KV option is chosen, **Key Vault Crypto User** (a key op) — not Secrets User — is
> the narrowest role; the verification-only option needs **no** runtime KV role at all.
...
- **api** → `identityApi` → **AcrPull · Queue Data Contributor · KV Secrets User · Blob Data Contributor · Table Data Contributor** — ✅ *(state-writer; KV unused #85; Azure state backend defaults local R6/#81)*
...
- **audit event store** → **`identityApi`** (active writer; worker grant unexercised #97) → **Storage Table Data Contributor** (append) — ✅ *hash-chain tamper-evident vs a trusted HEAD #59; co-located mutable HEAD + WORM #81; coverage partial/fail-open emit #99; **local SQLite by default***
...
- **Packs signing key** → **Key Vault Crypto User** *(pending #89 — KV lacks Ed25519; may be verification-only with no runtime KV role)* — 🔒 #89
...
depth for scaling), **Key Vault secret injection** (KV Secrets User deployed, but config is env-var
based — #85), the in-app Azure **Monitor** connector (roles deployed, but no workspace/resource env),
...
**Pending decision (🔒):** the pack signing trust root is tracked by **#89** (Azure Key Vault Keys
does not support Ed25519, so the design is being reconciled — verification-only with distributed
public keys, or ECDSA P-256 for KV-HSM signing); **Key Vault Crypto User** is the narrowest role
*only if* the KV-signing option is chosen.
...
value-scrubbed**, so their redaction (plus any future extra/tag fields) is tracked as **#91**. Key Vault secret injection is unimplemented so config is env-var based (**#85**). Audit
