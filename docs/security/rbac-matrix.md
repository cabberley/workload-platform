...
| **`identityWorker`** | the six capability modules — services `aiops`, `alerts` and jobs `discovery`, `quality_checks`, `reassessments`, `dependency_graph` | **compute-only** — reads state via the API and POSTs results to the API (the single writer); holds **no** Blob/Table write role (#97). Its role set is the **union** of the read-plane roles those six modules need — they are not individually identity-isolated |
...
**`api` is the *only* state-writer (issue #97).** Only the `api` identity is granted the
state-store write data roles (Storage Blob Data Contributor + Storage Table Data Contributor at the
storage-account scope — [`core.bicep`](../../infra/bicep/modules/core.bicep)). The **worker** is
**compute-only**: it reads prior state via the API (a read-only `ApiStateReader`) and POSTs its
module result to the API — the single code path that commits state — so it holds no Blob/Table write
role. This is **RBAC-enforced, not a convention**: the `worker`, `web` and `grafana` identities are
deliberately excluded, and a post-deploy CD gate ([`scripts/cleanup_verify_state_writers.py`](../../scripts/cleanup_verify_state_writers.py),
run from [`release.yml`](../../.github/workflows/release.yml) with `--allow $API_PID` only) removes
any non-`api` principal holding **one of three enumerated built-in state-write roles** at the storage
account and **fails the release fail-closed** if one remains. The gate's enforced `STATE_WRITE_ROLE_IDS` set is
...
> Reader (RG), Monitoring Reader (RG), Log Analytics Reader (workspace); plus the **api-only** state-write
> **assignments** Storage Blob/Table Data Contributor (storage — issue #97, the worker no longer holds them)). **A ✅ on a role means the *assignment* is deployed — not that the consumer
...
| **api** (service) | `identityApi` | *(intended)* The **single writer**: dispatch work to queues, read runtime secrets, **write state** (Blob/Table) — *state persistence defaults to the **LOCAL** SQLite backend (§api status) and **no client enqueues** to the queues (only KEDA polls depth); **Key Vault secret injection is now wired** (#85): `$WP_KEY_VAULT_URI` is threaded so the app-side provider resolves runtime secrets by identity (no api-specific required secret wired yet)* | **AcrPull** (`7f951dda-…`) · **Storage Queue Data Contributor** (`974c5e8b-…`) · **Key Vault Secrets User** (`4633458b-…`) · **Storage Blob Data Contributor** (`ba92f5b4-…`) · **Storage Table Data Contributor** (`0a9a7e1f-…`) | AcrPull = the ACR; Queue/Blob/Table = the platform storage account; KV Secrets User = the platform Key Vault | The API is the **sole** state-writer (issue #97), so it is granted the Blob + Table Data Contributor data roles (Contributor ⊇ Reader; the store creates and updates blobs/entities). Queue Data Contributor covers KEDA depth reads **and** the *intended* enqueue (no producer client enqueues today). KV Secrets User (get/list only) backs the keyless secret provider (**#85**, now used — the `aiops` worker consumes `system-pulse-read-token`; the api provider is enabled for future config). AcrPull to pull its image. Nothing narrower writes state. | Least privilege · keyless · provenance | ✅ *(role assignments deployed #80; KV injection wired #85; Azure state backend defaults local — R6/#81; no enqueue client)* |
| **worker** (identity — the six modules above) | `identityWorker` | **Aggregate (union) role set** for all six capability modules — **compute-only, no state write** | **AcrPull** (`7f951dda-…`) · **Storage Queue Data Contributor** (`974c5e8b-…`) · **Key Vault Secrets User** (`4633458b-…`) · **Reader** (`acdd72a7-…`) · **Monitoring Reader** (`43d0d8ad-…`) · **Log Analytics Reader** (`73c42c96-…`) | AcrPull/Queue/KV = storage/ACR/KV as for api; Reader + Monitoring Reader = RG; LA Reader = the workspace | **Compute-only — holds NO Blob/Table write role (issue #97).** The worker ([`worker.py`](../../src/cli/worker.py)) reads prior state via the API (a **read-only `ApiStateReader`**) and POSTs its module result to the API — the single code path that commits state — so it needs no state-write data role, and the "API is the sole state writer" boundary is now **RBAC-enforced** (only `api` may hold a Blob/Table write role). It retains only what it genuinely uses: AcrPull (image pull), Queue Data Contributor (KEDA depth), KV Secrets User (`system-pulse-read-token`), and the read-plane roles its connectors need (ARG Reader, aiops Monitoring/LA Reader). **Honest caveat:** the six modules share this one identity, so component-level separation *within* the worker is not achieved — finer per-module split is a candidate follow-up. | Least privilege *(per-identity, not per-module)* · keyless | ✅ *(all assignments deployed #79/#80; worker holds no state-write role — #97; several consumer flows ⚠️)* |
...
identities. It **also** runs the post-deploy API-only-writer gate
([`scripts/cleanup_verify_state_writers.py`](../../scripts/cleanup_verify_state_writers.py), invoked
with `--allow $API_PID` only — the worker is compute-only and holds no state-write role, #97), whose
...
- **quality_checks** → `identityWorker` → *(compute-only; persists via the API; no own cloud read, no state-write role)* — ✅
- **reassessments** → `identityWorker` → *(compute-only; persists via the API; no own cloud read, no state-write role)* — ✅
...
- **api** → `identityApi` → **AcrPull · Queue Data Contributor · KV Secrets User · Blob Data Contributor · Table Data Contributor** — ✅ *(sole state-writer #97; KV injection wired #85; Azure state backend defaults local R6/#81)*
- **worker** → `identityWorker` → **AcrPull · Queue Data Contributor · KV Secrets User · Reader · Monitoring Reader · Log Analytics Reader** — ✅ *(compute-only, NO Blob/Table write role #97; read-plane union of the six modules)*
...
- **audit event store** → **`identityApi`** (sole writer; worker holds no state-write role #97) → **Storage Table Data Contributor** (append) — ✅ *hash-chain tamper-evident vs a trusted HEAD #59; co-located mutable HEAD + WORM #81; coverage complete/fail-closed emit #99 (ADR 0014); **local SQLite by default***
...
**The only state-writer is `api`** (Storage Blob + Table Data Contributor) — the worker is
**compute-only** and holds no Blob/Table write role (**#97**), so the **API is the sole state
writer**. Every other runtime
identity is read-only: `worker` = read-plane roles only (AcrPull · Queue · KV Secrets · Reader ·
...
per-component identities (**#79**, merged) plus the API-only-writer tightening (**#97**) now enforce
the **API-only state-writer** boundary by RBAC — the old shared-identity gap is **resolved** and the
worker's former Blob/Table grant is **removed**; the six worker-hosted modules still share one
identity, so finer per-module separation is a candidate follow-up. The **worker is compute-only** —
it reads and writes state only *via* the API through a read-only `ApiStateReader` and holds **no**
Blob/Table write role, so the **API is the sole state writer (#97 — RBAC-enforced)**. The queue grant
is broader than current
