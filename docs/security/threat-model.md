...
| **Pack integrity / signing** | Packs are hashed + signed and verified **before** execution. | [`shared/signing.py`](../../src/shared/signing.py); [`packs_engine/engine.py`](../../src/packs_engine/engine.py); [`pack-validate.yml`](../../.github/workflows/pack-validate.yml) | **Largely NOT enforced.** Integrity fields are **optional**; the engine checks a hash/signature **only if the pack supplies one**; **CI signing is conditional** (trust root disabled → unsigned packs pass); shipped seed packs carry no hash/signature; **runtime wires no verifier**. Detached Ed25519 is *implemented* but not enforced, the KV providers are stubs, and **KV Keys does not support Ed25519** (**R1**; trust-root reconciliation **#89**, require integrity fields **#82**) |
...
1. **Packs-IN (signed content boundary).** Microsoft-authored, **signed** content packs flow *in*.
   *Intended* to be signature-verified before execution; today the runtime loads **bundled local
   `content/` files unverified** (**R1** — verifier/trust-root **#89**, integrity fields **#82**,
   registry import **#37**).
...
graph LR
    ...
    subgraph CUST[Customer subscription — in-boundary]
        ...
        KV[(Key Vault - not wired at runtime)]
    ...
    MOD -. read token env var; not wired in deployed jobs .-> SP
    ...
    API -. KV read: intended, not wired .-> KV
```
...
> - **Pack source:** runtime loads **bundled local `content/` files** as-is — there is **no** live
>   registry import/resolution and **no** verifier ([`wiring.py`](../../src/cli/wiring.py) builds
>   `PacksEngine(root)`). Registry import/assignment (**R1**, #37) and signature verification
>   (trust-root/verifier **#89**) are **intended but unimplemented**; inbound packs are
>   **unsigned/unhashed** today (**#82**). Digest-addressed content storage + digest-loaded runtime
>   resolution is a separate tracked item (#44).
...
> - **Key Vault:** `API → KV` secret injection is **intended, not wired** — runtime config/tokens are
>   read from **environment variables** today; the KV signing providers are fail-closed stubs, and the
>   KV-Ed25519 trust-root design is being reconciled (**R1**, #89; secret injection **#85**).
...
- **B3 · Azure Monitor telemetry ingest.** `azure-monitor-query` is present and **Monitoring Reader /
  ...
  not.
- **B4 · System Pulse telemetry.** Requires `SYSTEM_PULSE_BASE_URL` + `$SYSTEM_PULSE_READ_TOKEN`;
  runtime supplies **no credential provider** and the token is env-var based pending Key Vault
  injection (**#85**).
...
- **B7 · Pack integrity / signing enforcement + signed registry import.** The detached Ed25519
  verifier is *implemented* ([`signing.py`](../../src/shared/signing.py)) but **partly wired**: CI
  verifies a signature only if present (trust root disabled → unsigned packs pass) and **runtime wires
  no verifier**. The Key Vault signing providers are **stubs**, and the recorded "KV Ed25519 keyless"
  trust-root design is **not viable as stated — Azure Key Vault Keys does not support Ed25519**.
  Reconciling the trust root (customer-side **verification-only** with distributed Ed25519 **public**
  keys + offline Microsoft signing; **or** **ECDSA P-256** for KV-HSM signing) plus wiring the
  verifier is tracked by **#89**. Pack import/assign is **#37**; requiring integrity fields is **#82**
  (**R1**).
...
- **B10 · Key Vault secret injection** for runtime config/connector tokens — **R12**, #85.
...
|--------|---------------------|-----------|
| A forged or attacker-authored pack is imported and executed. | Detached Ed25519 verification is *implemented* in the engine, but it is **not enforced**: integrity fields are optional, the engine checks a hash/signature only if the pack supplies one, **CI does not require signing** (trust root disabled → unsigned packs pass), shipped seed packs are unsigned/unhashed, and **runtime wires no verifier**. So an unsigned/tampered pack currently **loads** ([`engine.py`](../../src/packs_engine/engine.py), [`contracts.py`](../../src/shared/contracts.py), [`pack-validate.yml`](../../.github/workflows/pack-validate.yml)). **There is also no runtime registry resolution** — the engine loads **bundled local `content/` files** as-is; a signed registry import is intended but unimplemented (import **#37**; signing trust root **#89**). | Pack signing *(not enforced; #89, #82)* |
| A module or connector authenticates with a leaked static key. | No static Azure keys exist: every **Azure** client is keyless via `DefaultAzureCredential` under its **own per-component identity**, the storage account disables shared-key access, and CI blocks committed secrets. **Qualified:** app-plane connector tokens (System Pulse) are read from **environment variables**, not Managed Identity ([`connectors/base.py`](../../src/shared/connectors/base.py)); Key Vault secret injection is not wired (**#85**). | Keyless *(per-component identities; env-var tokens for app-plane)* |
...
| Threat | Mitigation / status | Guardrail |
|--------|---------------------|-----------|
| A pack is modified in transit or at rest after signing. | The signature *would* catch this. **But it is not enforced:** CI does not require signing (unsigned packs pass), integrity fields are optional, shipped packs are unsigned, and runtime injects no verifier — so a modified/unsigned pack loads (**GAP R1**; verifier/trust-root wiring **#89**, require integrity fields **#82**). | Pack signing *(not enforced)* |
...
| A bearer token or secret is logged or embedded in code. | Tokens are resolved at the edge and returned only to the immediate caller (never logged); config/token **values are read from environment variables today** (Key Vault secret injection intended — **#85**), never embedded as literals; CI secret-scan + guardrail grep block committed secrets. | Keyless *(no static keys; KV injection pending #85)* |
...
|---|---------------------|--------|-----------|
| **R1** | **Pack integrity/signing not enforced.** Integrity fields are optional; the engine verifies a hash/signature only if the pack supplies one; **CI signing is conditional** (trust root disabled → unsigned packs pass); shipped seed packs are unsigned/unhashed; and **runtime wires no verifier**. Detached Ed25519 is implemented but not enforced, and the KV signing providers are stubs. The runtime loads **bundled local `content/` files as-is** — no registry resolution. The recorded "KV Ed25519" trust root is **not viable as stated** (Azure Key Vault Keys does not support Ed25519). | Implemented, not enforced | Registry import/assign **#37**; digest-addressed content store + digest resolution **#44**; require integrity fields **#82**; **signing trust root (Ed25519-vs-KV reconciliation) + verifier wiring #89** |
...
| **R12** | **Key Vault secret injection unimplemented.** Runtime config and connector tokens are read from **environment variables**, not Key Vault — `cfg` defaults to `os.environ` ([`wiring.py`](../../src/cli/wiring.py)) and the Bicep declares no `secretRef`/`secrets`. The **Key Vault Secrets User** role (on `api`/`worker`) is provisioned but currently unused. | Provisioned, not used | Runtime secret injection **#85** |
