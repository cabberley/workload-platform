...
- **A8 · Per-component identities enforce an *API-only*-writer boundary (#79/#97).** There is **no
  ...
  equivalent Blob/Table write `dataActions` is **not** detected (residual **#98**). **The boundary is
...
| **R4** | **Per-component identities + API-only writer — RESOLVED (merged #79/#97).** The platform now runs under **four** user-assigned identities (`identityApi`, `identityWorker`, `identityWeb`, `identityGrafana`) — **no shared runtime identity**. Only `api` holds the state-write roles; `worker` (compute-only), `web` and `grafana` are excluded, so the **API-only-writer boundary is RBAC-enforced**, and a post-deploy CD gate ([`cleanup_verify_state_writers.py`](../../scripts/cleanup_verify_state_writers.py), `--allow $API_PID` only) removes any stray principal holding **one of the three enumerated built-in state-write roles** and fails the release fail-closed (a **custom RBAC role** granting equivalent write `dataActions` is **not** detected — residual **#98**). **#97 closed the last gap:** the worker's former Blob/Table Data Contributor grant is **removed** (bicep) and dropped from the CD allow-list, so the worker — which is compute-only and writes via the API through a read-only `ApiStateReader` ([`worker.py`](../../src/cli/worker.py)) — can no longer write state directly and bypass API validation. **Residual:** the six capability modules share the `worker` identity, so finer *per-module* separation is not yet achieved. | **Resolved** (API-only writer RBAC-enforced #79/#97); custom-role gate coverage #98; per-module split outstanding | Per-component identities **#79** (merged); API-only writer **#97** (this change); gate custom-role `dataActions` coverage **#98**; per-module split = *untracked — candidate follow-up* |
