...
| **Least privilege** (#7) | Each component requests the narrowest Azure RBAC role that works; document why. | Per-component identities + scoped role assignments ([`core.bicep`](../../infra/bicep/modules/core.bicep), [`grafana.bicep`](../../infra/bicep/modules/grafana.bicep)); [`rbac-matrix.md`](rbac-matrix.md) | **Enforced at the identity level (#79/#80/#97).** Four identities: `api` is the only state-writer; `worker` is compute-only (reads + writes via the API, holds no Blob/Table role — #97); `web` = AcrPull only; `grafana` = Monitoring/LA Reader only. The six worker-hosted modules share one identity (finer per-module split is a follow-up) and the queue grant is broader than needed today |
| **Provenance** (#8) | Findings cite evidence; audit records are tamper-evident. | `enforce_finding_provenance` ([`shared/provenance.py`](../../src/shared/provenance.py)) called from every state write path ([`shared/state.py`](../../src/shared/state.py) `_write_findings`) + `run_module`; `Finding` contract ([`shared/contracts.py`](../../src/shared/contracts.py)); tamper-evident audit hash chain ([`shared/audit.py`](../../src/shared/audit.py), #59) | **Largely enforced.** **Evidence provenance IS required and fail-closed** (#59): `enforce_finding_provenance` runs at the module-emission boundary (`run_module`) **and** at every durable write (`_write_findings` on **both** the local SQLite and Azure Table backends), so a finding with no attributable `sourceReference` is rejected and the whole transaction rolls back **before** any row is written. Only **`packId`/`packVersion` remain optional** (some findings — e.g. dependency_graph — derive from the estate/graph, not a pack), so pack/version provenance enforcement is the remaining follow-up (**#83**). Audit records are additionally **hash-chained and tamper-evident on read *relative to a trusted / out-of-band HEAD*** (#59 — §A10; co-located mutable HEAD residual #81). **Audit *coverage* is now complete and *fail-closed*:** `run.executed`, finding-emitted, **and** the state-mutating `put_estate`/`put_graph`/`snapshot` endpoints all emit an audit event, and for these security-material actions emission is **fail-closed** — a durable-append failure raises `AuditPersistenceError` so the mutating write fails (5xx) rather than proceeding unrecorded, and the failure is surfaced on `audit_emit_failures_total` (**#99**, [ADR 0014](../adr/0014-fail-closed-audit-emission.md); the `pack.verify`-failure breadcrumb keeps a narrow, documented fail-open allowance). Only the three state-mutation subjects (`wl:<digest>#…`) are **PII-free by construction** (opaque full-SHA-256 workload token); `finding.emitted` still carries a raw `<workload>#count=N` (sole residual, deferred to **#65**), and `run.executed` carries the **module identifier** (not the workload) |
| **Append-only audit** | Audit/state history is not silently rewritten. | Hash-chained audit trail ([`shared/audit.py`](../../src/shared/audit.py), #59); commit-id-scoped state blobs ([`shared/state.py`](../../src/shared/state.py)) | **Tamper-evident (audit) — #59 merged.** Every audit record chains `sha256(canonical ‖ prevHash)` with an anchored HEAD, so edits/reorder/truncation are **detectable on read *relative to a trusted HEAD*** (`verify_audit_chain`). **Limitation:** the event rows and the chain HEAD are **co-located in one mutable Table partition** (Azure backend; local SQLite by default), so a state-write-role holder can **coordinate-rewrite history and the HEAD** consistently and evade detection. Storage-layer immutability (WORM/versioning) **plus an out-of-band HEAD anchor** is still a follow-up (**R6**, #81). **Coverage is now complete and fail-closed:** `run.executed`, finding-emitted, **and** the state-mutating `put_estate`/`put_graph`/`snapshot` endpoints all emit an audit event, and emission is **fail-closed** for these security-material actions (a durable-append failure fails the mutating write 5xx rather than proceeding unrecorded — **#99**, [ADR 0014](../adr/0014-fail-closed-audit-emission.md)) |
...
graph LR
    ...
    subgraph CUST[Customer subscription — in-boundary]
        ...
        ST[(State: sqlite LOCAL active; Blob+Table AZURE role-granted to api only #97 but backend not wired)]
        ...
        IDW((identityWorker - compute only))
...
>   writer), `identityWorker` (the six modules, **compute-only** — reads + writes via the API,
>   holds no Blob/Table write role, #97), `identityWeb` (web, AcrPull only),
>   `identityGrafana` (Grafana, read-only). The **API-only-writer boundary is RBAC-enforced** (#97)
...
>   at runtime. The **api's Storage Blob/Table Data Contributor** grants **are deployed** (#80; api-only
>   since #97) — the *role* is provisioned, the *consumer flow* stays bucket B (§B6); storage-layer
>   immutability is #81. The worker holds **no** state-write role (#97).
...
| **B5** | Compute modules ⟷ state (single-writer) | The six modules (`identityWorker`) vs the API core (`identityApi`) / state store | **The only execute+persist path today is the API `/run` endpoint** (unauthenticated — **R3**, #64). **Deployed job workers do not submit results** — they run with `--module` only (no `--scope workload=`), and `worker.py` submits only when a workload scope exists; the **service modules only heartbeat**. Job result-submission is **intended, not wired**. The **API-only-writer boundary is RBAC-enforced** (#79/#97): only `identityApi` holds the state-write roles (Blob/Table Data Contributor); the `worker` (compute-only), `web` and `grafana` identities are excluded, and a post-deploy CD gate fails closed if any other principal holds a state-write role — EITHER **one of the three enumerated built-in state-write roles** OR a **custom RBAC role** whose effective `dataActions` grant equivalent Blob/Table write/delete (**#98**; management/control-plane roles out of scope while `allowSharedKeyAccess=false`). | Through the API |
...
- **A3 · State persistence today = the LOCAL backend.** `build_state_store()` defaults to `local`
  ...
  **ephemeral**. The **API-only-writer boundary is RBAC-enforced** (#79/#97 — A8), not a
  convention.
...
- **A7 · Read-plane RBAC is provisioned (least-privilege, deployed).** The deploy grants, in Bicep,
  the **worker** identity: **Reader** (RG scope), **Monitoring Reader** (RG), **Log Analytics Reader**
  (workspace); the **grafana** identity its own **Monitoring Reader** + **Log Analytics Reader**; and
  `api` the writer set (incl. **Storage Blob + Table Data Contributor**, storage-account scope — api
  only since #97) ([`core.bicep`](../../infra/bicep/modules/core.bicep), [`grafana.bicep`](../../infra/bicep/modules/grafana.bicep),
  ...
  *not* granted** (B5).
- **A8 · Per-component identities enforce an *API-only*-writer boundary (#79/#97).** There is **no
  shared runtime identity**: `identityApi` (writer), `identityWorker` (**compute-only**),
  `identityWeb` (AcrPull only), `identityGrafana` (read-only). **What is RBAC-enforced:** *only* `api`
  may hold a state-write role — `worker`/`web`/`grafana` are excluded
  ([`core.bicep`](../../infra/bicep/modules/core.bicep)),
  and a post-deploy CD gate ([`cleanup_verify_state_writers.py`](../../scripts/cleanup_verify_state_writers.py),
  run from [`release.yml`](../../.github/workflows/release.yml) with `--allow $API_PID` only)
  removes any **other/stray** principal holding a state-write role — EITHER **one of three enumerated
  built-in state-write roles** (Blob Data Owner/Contributor, Table Data Contributor) OR a **custom
  RBAC role** whose effective `dataActions` grant equivalent Blob/Table write/delete (including
  wildcards such as `Microsoft.Storage/*` that expand to them; `notDataActions` revocations honoured)
  — and **fails the release fail-closed** if one remains (**#98**). Only DATA-plane actions are
  inspected: management/control-plane rights (`actions` — e.g. `storageAccounts/write`, key listing)
  are **out of scope** while `allowSharedKeyAccess=false`, and a role definition that cannot be
  resolved is treated fail-closed as a possible writer. **The boundary is
  now enforced end-to-end (#97):** the worker is **compute-only**
  ([`worker.py`](../../src/cli/worker.py)) — it builds a
  **read-only `ApiStateReader`** (HTTP reads, no write methods) and POSTs its result to
  ...
  Because the worker's former Blob/Table Data Contributor grant is now **removed** (bicep) and dropped
  from the CD allow-list, a compromised worker identity **cannot** write state directly and bypass the
  API's server-side validation — the "API is the sole state writer" invariant holds at the RBAC layer,
  not merely by code convention. *(The six modules still share the worker identity, so finer
  per-module separation is a candidate follow-up.)*
...
- **A10 · Tamper-evident audit trail — complete coverage, fail-closed emit (#59, #99).** Every audit
  record is linked into a hash chain — `entryHash = sha256(canonical_bytes(event) ‖ prevHash)` with an
  anchored genesis and a maintained chain HEAD ([`audit.py`](../../src/shared/audit.py)).
  `verify_audit_chain` recomputes the chain and detects **tampered fields, reorder/insertion/deletion,
  and tail-truncation** *relative to a trusted anchored HEAD* (see the Tampering row for the
  co-located-HEAD limitation). **Coverage is complete:** `run.executed`, finding-emitted/persisted,
  **and** the state-mutating `put_estate`, `put_graph` and `snapshot` endpoints all emit a
  bounded-subject audit event ([`main.py`](../../src/api/app/main.py), **#99**). **Emission is
  fail-closed and ordered audit-BEFORE-write** for these security-material actions: the durable
  `append_audit` runs FIRST and the state mutation happens **only if it succeeds**, so a hard
  `append_audit` failure raises `AuditPersistenceError` and the mutating write **fails (5xx) with no
  committed-but-unaudited state**, and the failure is surfaced on the PII-free
  `audit_emit_failures_total` counter
  ([`audit.py`](../../src/shared/audit.py), [ADR 0014](../adr/0014-fail-closed-audit-emission.md)). The
  three new subjects (estate/graph/snapshot) are **PII-free by construction** — they embed an opaque
  one-way `wl:<digest>` token (the **full** `sha256(workload)` hex, collision-resistant), never the
  raw caller-controlled workload name, so a workload id containing PII can never reach those durable
  subjects. (The pre-existing `finding.emitted` subject still carries the raw `<workload>#count=N` —
  deferred to **#65**; `run.executed` carries the module identifier, not the workload.) A **narrow,
  documented allowance** keeps the
  non-material `pack.verify`-failure breadcrumb best-effort (it is emitted mid-pack-load, and the pack
  is rejected fail-closed regardless). *Audit-before-write, not two-phase:* the safe residual is
  over-recording — a state write that fails *after* a successful audit append leaves a record with no
  state change (never the reverse). **The wired-today append path is the LOCAL SQLite backend** —
  `build_state_store` defaults to `local` and only uses
  Azure Table when `WORKLOADS_STATE_BACKEND=azure` is explicitly set (not set by the current bicep)
  ([`state.py`](../../src/shared/state.py) L1028-1036). The api's **Storage Table Data Contributor**
  role (append-for-provenance, guardrail #8) is therefore **deployed for the intended `azure` backend
    but not the active flow by default** (api only since #97; the worker holds no state-write role). Storage-layer immutability (WORM/versioning) and an out-of-band
  HEAD anchor that would *prevent* (not just detect) a coordinated rewrite are a follow-up (**R6**, #81).
...
- **B6 · Azure Blob/Table state backend.** The ARCHITECTURE target store. **Storage Blob + Table Data
  Contributor are assigned** (storage-account scope, #80 — to `api` only since #97), so the *role* is
...
| Threat | Mitigation / status | Guardrail |
|--------|---------------------|-----------|
| It is unclear which knowledge produced a finding, or a result cannot be attributed. | **Evidence provenance is required and enforced fail-closed** (#59): `enforce_finding_provenance` rejects any finding lacking an attributable `sourceReference` at the module-emission boundary (`run_module`) **and** at every durable write (`_write_findings` on both the local and Azure backends), rolling the whole transaction back before persistence ([`provenance.py`](../../src/shared/provenance.py), [`state.py`](../../src/shared/state.py)). The `Finding` contract additionally *supports* `packId`/`packVersion`, but those remain **optional** — some producers (dependency_graph) legitimately derive from the estate/graph rather than a pack ([`contracts.py`](../../src/shared/contracts.py), [`dependency_graph/module.py`](../../src/modules/dependency_graph/module.py)). **Audit coverage is now complete and fail-closed:** `run.executed`, finding-emitted, **and** the state-mutating `put_estate`/`put_graph`/`snapshot` endpoints all emit an audit event ([`main.py`](../../src/api/app/main.py)) — the three state-mutation subjects (`wl:<digest>#…`) are **PII-free by construction** (opaque full-SHA-256 workload token), while `finding.emitted` still carries a raw `<workload>#count=N` (sole residual, deferred to **#65**) and `run.executed` carries the **module identifier** (not the workload) — and audit emission is **fail-closed** for these security-material actions — a persistence failure **blocks and fails** the mutating write (5xx) rather than proceeding unrecorded, and is surfaced on `audit_emit_failures_total` ([`audit.py`](../../src/shared/audit.py) `AuditEmitter.emit`; **#99**, [ADR 0014](../adr/0014-fail-closed-audit-emission.md)). **Pack/version provenance on findings is not yet enforced** (**#83**). | Provenance *(evidence enforced #59; audit complete/fail-closed #99; pack/version not enforced #83)* |
...
| Threat | Mitigation / status | Guardrail |
|--------|---------------------|-----------|
| An identity holds more Azure rights than it needs. | Roles are scoped **per component** (#79/#80/#97, see [`rbac-matrix.md`](rbac-matrix.md)): four identities — `identityApi` is the **only state-writer** (Storage Blob + Table Data Contributor), `identityWorker` is **compute-only** (read-plane roles only; no Blob/Table write role — #97), `identityWeb` holds **AcrPull only**, `identityGrafana` holds **Monitoring/LA Reader only**. The API-only-writer boundary is **RBAC-enforced** and re-verified fail-closed by a post-deploy CD gate ([`cleanup_verify_state_writers.py`](../../scripts/cleanup_verify_state_writers.py)). **Residual:** the six capability modules share the **worker** identity, so finer *per-module* least privilege is not yet achieved (candidate follow-up), and the Storage Queue Data Contributor grant is broader than the KEDA-depth consumer needs (untracked follow-up). | Least privilege *(per-component #79/#80; API-only writer #97; per-module split outstanding)* |
| The platform mutates or remediates customer infrastructure. | **No customer-infrastructure mutation/remediation client is constructed** — ARG, Azure Monitor, System Pulse, and network topology are all **read-only**; remediation is advisory only ([ADR 0005](../adr/0005-advisory-remediation-in-ops-packs.md)). *(This excludes the platform's **own** state storage, writable only by the `api` identity — [`state.py`](../../src/shared/state.py) — scoped to the platform account, never customer data stores.)* | No auto-remediation · least privilege *(enforced for customer infra)* |
...
| **R4** | **Per-component identities + API-only writer — RESOLVED (merged #79/#97); gate custom-role coverage RESOLVED (#98).** The platform now runs under **four** user-assigned identities (`identityApi`, `identityWorker`, `identityWeb`, `identityGrafana`) — **no shared runtime identity**. Only `api` holds the state-write roles; `worker` (compute-only), `web` and `grafana` are excluded, so the **API-only-writer boundary is RBAC-enforced**, and a post-deploy CD gate ([`cleanup_verify_state_writers.py`](../../scripts/cleanup_verify_state_writers.py), `--allow $API_PID` only) removes any stray principal holding a state-write role and fails the release fail-closed. **#98 widened the gate's coverage:** a principal counts as a state-writer if it holds EITHER **one of the three enumerated built-in state-write roles** OR a **custom RBAC role** whose effective `dataActions` grant equivalent Blob/Table write/delete (including wildcards like `Microsoft.Storage/*`; `notDataActions` revocations honoured; unresolvable role definitions treated fail-closed as possible writers). Only DATA-plane actions are inspected — management/control-plane roles (key listing, `storageAccounts/write`) are out of scope while `allowSharedKeyAccess=false`. **#97 closed the last identity gap:** the worker's former Blob/Table Data Contributor grant is **removed** (bicep) and dropped from the CD allow-list, so the worker — which is compute-only and writes via the API through a read-only `ApiStateReader` ([`worker.py`](../../src/cli/worker.py)) — can no longer write state directly and bypass API validation. **Residual:** the six capability modules share the `worker` identity, so finer *per-module* separation is not yet achieved. | **Resolved** (API-only writer RBAC-enforced #79/#97; gate covers built-in + custom-role dataActions #98); per-module split outstanding | Per-component identities **#79** (merged); API-only writer **#97**; gate custom-role `dataActions` coverage **#98**; per-module split = *untracked — candidate follow-up* |
...
| **R9** | **Doc/code drift on pack integrity — PARTIALLY MITIGATED (#82).** [`ARCHITECTURE.md`](../../ARCHITECTURE.md), [`SECURITY.md`](../../SECURITY.md), and the authoring/skill docs now describe the IMPLEMENTED scheme honestly: a **SHA-256 content hash over canonical bytes** (whole manifest + body — [`signing.py`](../../src/shared/signing.py)/[`canonical.py`](../../src/packs_engine/canonical.py) canonicalization) verified before execute, plus a detached **Ed25519** signature that is the authenticity bar for **imported/third-party** packs, with the legacy HMAC MAC noted as an independent optional gate. The canonical content-hash field is now **REQUIRED fail-closed at the shipped-pack load boundary** ([`engine.py`](../../src/packs_engine/engine.py) `_require_shipped_integrity`), and a **present-but-unverifiable** first-party signature is rejected fail-closed (a present signature must verify, never silently accepted). **Honest scope:** **first-party / shipped packs currently receive HASH-ONLY integrity — tamper-evidence in transit, NOT authenticity.** Shipped packs are **not signed or signature-verified today**; first-party **authenticity enforcement is DEFERRED** behind a `TODO(human)` hook until the offline signing key / pinned trust root land (#37/#44). Imported/third-party packs ARE held to the verified-Ed25519 bar (#89). | **Partially mitigated** (docs reconciled + canonical content-hash required fail-closed); first-party authenticity (signature) deferred | Reconcile pack-integrity docs + require integrity fields **#82**; first-party signing key **#37/#44** |
...
| **R14** | **Audit coverage complete + fail-closed, audit-before-write emit (#99 — resolved).** All three state-mutating endpoints now emit a PII-free, bounded-subject audit event with an opaque `wl:<digest>` workload token (PII-free by construction) — `put_estate` (`wl:<digest>#estate=N`), `put_graph` (`wl:<digest>#graph=nodes=N,edges=M`), `snapshot` (`wl:<digest>#snapshot`) — alongside `run.executed`/finding-emitted ([`main.py`](../../src/api/app/main.py)); and `AuditEmitter.emit` is **fail-closed** for these security-material actions (`FAIL_CLOSED_ACTIONS`), recorded **audit-BEFORE-write**: the durable append runs first and the mutation runs only if it succeeds, so an `append_audit` failure raises `AuditPersistenceError`, the mutating write **fails 5xx with no committed-but-unaudited state**, and increments `audit_emit_failures_total` ([`audit.py`](../../src/shared/audit.py), [ADR 0014](../adr/0014-fail-closed-audit-emission.md)). A narrow, documented fail-open allowance remains for the non-material `pack.verify`-failure breadcrumb. **Residuals:** audit-before-write is not two-phase (a write that fails *after* a successful audit append over-records — the safe direction); the pre-existing `finding.emitted` subject still embeds the raw workload id (`run.executed` carries the module identifier, not the workload; workload-ID grammar hardening is owned by tenant-isolation **#65**); and dedicated `estate/graph/snapshot` `AuditAction` members are a follow-up contract change (they currently reuse `run.executed`, disambiguated by subject). | Coverage complete; fail-closed + audit-before-write for material actions; new subjects PII-free by construction | Workload-ID grammar (#65) + dedicated action members — *candidate follow-up* |

**Verdict.** This is a scaffold, and several guardrails are now genuinely enforced: **per-component
identities (#79)** together with the **API-only-writer** tightening (#97) make the **API-only-writer**
boundary **RBAC-enforced** — the worker is compute-only and writes via the API through a read-only
reader, and its former Blob/Table Data Contributor grant has been **removed** (bicep) and dropped from
the CD writer-gate allow-list, so `api` is now the **sole** identity holding a state-write role; the
**findings-OUT** crossing
