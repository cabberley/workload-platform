...
| Control objective | Platform control | Status | Evidence |
|---|---|---|---|
| No PHI/PII leaves the boundary | **No-PII-egress CI audit** — the audit imports the FastAPI app, enumerates the **real** response models from every route's `response_model` (and notification payloads), and **recursively** walks the full serialized schema graph (nested models, `list`/`dict`/`Optional`/`Union` members, computed fields, `RootModel`), checking each field's **effective emitted key** (alias / `serialization_alias`) against a PII denylist; CI fails closed if a PII-named or unclassified field is introduced | **Partial** | `scripts/audit_no_pii_egress.py`; `tests/unit/test_audit_no_pii_egress.py`; `src/api/app/main.py` (route `response_model`s); `src/modules/alerts/module.py` (`_notification_payload`). **Closed by #91:** every previously raw-`dict` route (`GET /api/health`, `GET /`, `GET /api/modules`, `POST …/results`, `…/estate`, `…/graph`, `…/findings`, `…/snapshot`) now returns a **bounded typed `response_model`**, and the metrics label maps are projected onto the bounded `MetricsSnapshotView` (allow-listed `MetricLabelKey` keys + `redact_value`-coerced values) at `/api/metrics`. **Residual gap:** four value-constrained open mappings remain and are held as loud `TRACKED WAIVER (#91)` entries — `ModuleRunResult.extra` (`dict[str, Any]`, heterogeneous internal module output), `ImpactResult.states` (`dict[str, HealthState]`, keys are graph node ids also egressed via `down`/`degraded`), `ResourceNode.tags` (customer Azure tags read internally by modules at full fidelity), and `ScaleTrigger.metadata` (`dict[str, str]` KEDA scaler config). The audit **flags every one fail-closed** and permits only an explicit, issue-referenced tracked waiver; the waiver count dropped from **13 → 4** for #91. **Default-redact at egress (#91 R3):** the two customer-controlled/-derived surfaces (`ResourceNode.tags` and `ModuleRunResult.extra`) are now **default-DENY redacted at the API response boundary** — every value → `[redacted]`, every customer key → an opaque positional placeholder, every unsupported leaf type redacted — so the remaining waivers cover only the open *type shape*, not actual PII egress; the handler-bypass detector resolves each route's return to its **exact reviewed projection** (by identity/structural proof) and fails closed with an **unwaivable** `<unredacted egress>` violation (evaluated even under a route-level unbounded waiver) if any route hands back an unsanitized `ResourceNode`/`ModuleRunResult`, a partially-sanitized collection, or a decoy wrapper. Fully closing these last four *type shapes* (bounding the key types) moves this control to **Implemented** |
...
| Egress-value hardening (opaque ids + default-redact) | `ResourceNode.tags` (keys **and** values) and the nested `ModuleRunResult.extra` free-form surface are now **default-redacted at the API response boundary** (#91, default-DENY): every tag/`extra` value → `[redacted]`, every customer-/workload-derived mapping **key** → an opaque positional placeholder (`redacted_key_N`), and any unsupported leaf type (`str`/`bytes`/nested Pydantic model/dataclass/`set`) is redacted rather than serialized — so PHI/PII (patient names, MRNs, SSNs, emails) can never egress verbatim as a value **or** a key; only platform/module-defined schema keys (`PLATFORM_SAFE_STRUCTURAL_KEYS`) and bounded scalars (numbers/bools/`None`/enums) survive. Internal storage and impact/graph analysis keep RAW values (redaction is applied to an egress copy only). Free-text `Finding.title`/`detail` and opaque-finding-id enforcement remain separate value-level items | **Partial** | **default-redact implemented in #91**: `src/shared/contracts.py` (`redact_tree`, `redact_node_tags`, `PLATFORM_SAFE_STRUCTURAL_KEYS`/`PLATFORM_SAFE_TAG_KEYS` allow-lists, positional `redacted_key_N` placeholders), wired at egress in `src/api/app/main.py` (`_redact_run_result_for_egress`, `_EstateEgress.redact`), enforced by `scripts/audit_no_pii_egress.py` (unwaivable `<unredacted egress>` fail-closed check); `Finding.title`/`detail` opaque-id hardening still tracked by **#78** |
| Egress key-text residual (flat allow-list vs customer key text) | `PLATFORM_SAFE_STRUCTURAL_KEYS` is a **flat** set of generic platform/module schema tokens (`drift`, `cadence`, `summary`, `detail`, `workload`, `newFailures`, …) matched by **exact string equality** against mapping keys (no path/type context). After the #91 R4 default-DENY value rule, the VALUE under any key is **always redacted** unless the key is exact-allow-listed, so no free-form value can egress; the residual is narrower — a **customer-authored KEY whose text happens to exactly equal one of these generic tokens** (e.g. a customer workload literally named `cadence` used as a map key in `extra`) would egress as that **key string only**. Blast radius is the key text itself (bounded to a platform-token spelling); its associated value is still `[redacted]`. The definitive fix — **path-scoped / typed egress schemas** that only allow a schema token where the module actually defines it (rather than a flat global set) — is deferred as a follow-up | **Partial** | flat allow-list + exact-match keys: `src/shared/contracts.py` (`PLATFORM_SAFE_STRUCTURAL_KEYS`, `_redact_key` exact-equality match, positional `redacted_key_N`); value always redacted under any key by the R4 Mapping rule (`redact_tree`); typed/path-scoped egress schemas tracked as a #91 follow-up |
...
| `ResourceNode` | API read model | `id`, `name`, `type`, `workload`, `tier`, `role`, `tags` | Resource ids/classifiers. `tags` is an unbounded `dict[str, str]` mapping whose type shape stays tracked by **#91**, but at the API egress boundary its keys **and** values are now **default-redacted** (`redact_node_tags`: values → `[redacted]`, customer keys → opaque `redacted_key_N` placeholders) so no customer tag PII egresses; `name` **value** remains a value-level hardening gap (#78) |
...
recursed for PII; only free-form `str`/`Any` keys remain unbounded. Issue #91 bounded the raw-`dict`
routes and the metrics label maps, cutting the #91 waivers from 13 to 4; the four value-constrained
residuals below are why the §09 / §13 egress controls are still **Partial**, not Implemented.
...
on those **values** is tracked by **#78**. The raw-`dict` routes and metrics label maps have now been
bounded (issue #91); the remaining value-constrained open mappings (`ResourceNode.tags`,
`ModuleRunResult.extra`, `ImpactResult.states`, `ScaleTrigger.metadata`) stay tracked by **#91**.
...
- **#91** — bound/redact externally-returned free-form mappings & raw-`dict` endpoints. **Largely
  delivered:** every raw-`dict` route now returns a bounded typed `response_model` and the metrics
  label maps are projected onto the bounded `MetricsSnapshotView` (allow-listed keys + redacted
  values), cutting the #91 waivers from 13 to 4. The four remaining value-constrained open mappings
  (`ResourceNode.tags`, `ModuleRunResult.extra`, `ImpactResult.states`, `ScaleTrigger.metadata`)
  stay held as loud `TRACKED WAIVER (#91)` entries with documented justifications.
