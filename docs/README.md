...
- **`delivery/`** — customer / MSP delivery playbooks.
  - **`delivery/customer-deployment.md`** — the **guided, keyless customer deploy** (Phase 1, issue #67): `azd` or the scripted `infra/deploy/` path to deploy the whole platform into the customer's **own** subscription (prerequisites, least-privilege roles, parameters, step-by-step, post-deploy verification). Turnkey self-service is scaffolded in `infra/marketplace/` (Phase 2).
  - **`delivery/lighthouse-onboarding.md`** — **Azure Lighthouse** delegated resource management over **customer-owned** deployments (least-privilege read-only roles, keyless, boundary-preserving) with deploy/audit/revoke steps. See ADR `0011`.
